...
export default function SingleInvoice(props) {
  ...
  const getStatus = () => {
	...
	// TODO: Find out what this is and refactor, why is it needed???? 
  ...
  const statusToggle = (e) => {
    ...
    if (status === "Pending" || status === "Overdue") {
      setDoc(doc(db, "users", currentUser.email, "Invoices", id), { status: "Paid" }, { merge: true })
    } else if (status === "Paid" || status === "Draft") {
      setDoc(doc(db, "users", currentUser.email, "Invoices", id), { status: "Pending" }, { merge: true })
    }
  }
  ...
  const editToggle = async () => {
    if (!editDisable) {
      ...
      const valuesOnly = Object.keys(itemsStore[`invoice-${invoiceID}`]).map((val) => itemsStore[`invoice-${invoiceID}`][val])
      await setDoc(doc(db, "users", currentUser.email, "Invoices", `invoice-${invoiceID}`), { ...formData, items: valuesOnly }, { merge: true })
      // dispatch(clearItems())
  ...
  const getTotal = () => {
    if (itemsStore) {
      const foo = Object.values({ ...itemsStore[`invoice-${invoiceID}`] }).reduce((t, { total }) => t + total, 0)
        ...
        <div className="full-info-container">
          <div className="full-info-title-container">
            ...
            <div className="client-address-info">
							{/* TODO: Fix client email positioning */}
